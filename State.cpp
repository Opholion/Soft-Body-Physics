...
ID3D11BlendState* gSubtractiveBlendState = nullptr;
...
bool CreateStates()
{
    ...
    D3D11_BLEND_DESC AlphaBlendDesk = {};
    ////-------- Blending Off State --------////
    AlphaBlendDesk.RenderTarget[0].BlendEnable = TRUE;              // Disable blending
    AlphaBlendDesk.RenderTarget[0].SrcBlend = D3D11_BLEND_SRC_ALPHA;      // How to blend.
    AlphaBlendDesk.RenderTarget[0].DestBlend = D3D11_BLEND_INV_SRC_ALPHA;     // Blend with the pixels with that position on the screen.
    AlphaBlendDesk.RenderTarget[0].BlendOp = D3D11_BLEND_OP_ADD;   // Combine the first two. Additive blending

    AlphaBlendDesk.RenderTarget[0].SrcBlendAlpha = D3D11_BLEND_ONE;
    AlphaBlendDesk.RenderTarget[0].DestBlendAlpha = D3D11_BLEND_ZERO;
    AlphaBlendDesk.RenderTarget[0].BlendOpAlpha = D3D11_BLEND_OP_ADD;
    AlphaBlendDesk.RenderTarget[0].RenderTargetWriteMask = D3D11_COLOR_WRITE_ENABLE_ALL;

    // Then create a DirectX object for the description that can be used by a shader
    if (FAILED(gD3DDevice->CreateBlendState(&AlphaBlendDesk, &Transparency)))
    {
        gLastError = "Error creating alpha-blend state";
        return false;
    }

    D3D11_BLEND_DESC SubtractBlendDesk = {};
    ////-------- Blending Off State --------////
    SubtractBlendDesk.RenderTarget[0].BlendEnable = FALSE;              // Disable blending
    SubtractBlendDesk.RenderTarget[0].SrcBlend = D3D11_BLEND_ONE;      // How to blend the source (texture colour)
    SubtractBlendDesk.RenderTarget[0].DestBlend = D3D11_BLEND_ZERO;     // How to blend the destination (colour already on screen)
    SubtractBlendDesk.RenderTarget[0].BlendOp = D3D11_BLEND_OP_SUBTRACT;   // How to combine the above two, almost always ADD

    //** Leave the following settings alone, they are used only in highly unusual cases
    //** Despite the word "Alpha" in the variable names, these are not the settings used for alpha blending
    SubtractBlendDesk.RenderTarget[0].SrcBlendAlpha = D3D11_BLEND_ONE;
    SubtractBlendDesk.RenderTarget[0].DestBlendAlpha = D3D11_BLEND_ZERO;
    SubtractBlendDesk.RenderTarget[0].BlendOpAlpha = D3D11_BLEND_OP_SUBTRACT;
    SubtractBlendDesk.RenderTarget[0].RenderTargetWriteMask = D3D11_COLOR_WRITE_ENABLE_ALL;

    // Then create a DirectX object for the description that can be used by a shader
    if (FAILED(gD3DDevice->CreateBlendState(&SubtractBlendDesk, &gSubtractiveBlendState)))
    {
        gLastError = "Error creating Subtract-blend state";
        return false;
    }
...
void ReleaseStates()
{
    ...
    if (gSubtractiveBlendState)     gSubtractiveBlendState->Release();
