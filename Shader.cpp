...
const std::string ARR_VS_SHADERNAMES[ShaderCount] = { "ShadowMapping_vs" ,"GreenWiggleSphere_vs", "ShadowCubeMapping_vs" };//, "NormalMapShader_vs", "ReflectiveShader_vs", "AlphaBlendShader_vs" };
const std::string ARR_PS_SHADERNAMES[ShaderCount] = { "ShadowMapping_ps" ,"GreenWiggleSphere_ps", "ShadowCubeMapping_ps" };//, "NormalMapShader_ps", "ReflectiveShader_ps", "AlphaBlendShader_ps" };

ID3D11VertexShader* gPixelLightingVertexShader[ShaderCount] = { nullptr,nullptr,nullptr };
ID3D11PixelShader* gPixelLightingPixelShader[ShaderCount] = { nullptr,nullptr,nullptr };
...
bool LoadShaders()
{
    ...
    {
        gPixelLightingVertexShader[i] = LoadVertexShader(ARR_VS_SHADERNAMES[i]); // Note how the shader files are named to show what type they are
        gPixelLightingPixelShader[i] = LoadPixelShader(ARR_PS_SHADERNAMES [i]);
    }
    ...
    for (int i = 0; i < ShaderCount; ++i)
    {
        if (gPixelLightingVertexShader[i] == nullptr || gPixelLightingPixelShader[i] == nullptr)
        {
            gLastError = "Error loading shaders, " + i;
            return false;
        }
    }

    if (gBasicTransformVertexShader == nullptr || gLightModelPixelShader    == nullptr || gDepthOnlyPixelShader == nullptr)
    {
...
void ReleaseShaders()
{
    ...
    for (int i = 0; i < ShaderCount; ++i)
    {
        if (gPixelLightingPixelShader[i])    gPixelLightingPixelShader[i]->Release();
        if (gPixelLightingVertexShader[i])   gPixelLightingVertexShader[i]->Release();
    }
}
